...
public class Ular extends Actor
{
    ...
    private void setGambar(int d)
    {
     ...
     private void setGambar(int width, int height) {
    ...
    private void setGambar(int width, int height, Color color) {
        GreenfootImage image = new GreenfootImage(width, height);
    ...
    public void act() 
    {
        if(((MyWorld)getWorld()).GAMEOVER)
        return;
        posx=getX();posy=getY();
        if(ular!=null){
            if((getX()==ular.posx && getY()==ular.posy)||(getX()==ular.getX() && getY()==ular.getY())){
            }else 
            setLocation(ular.posx,ular.posy); 
        }else{
            int px=getX()+arahx*delta;
            int py=getY()+arahy*delta;
            if(px>getWorld().getWidth()+delta)
            px=-delta;
            if(px<-delta)
            px=getWorld().getWidth()+delta;
            if(py>getWorld().getHeight()+delta)
            py=-delta;
            if(py<-delta)
            py=getWorld().getHeight()+delta;
            setLocation(px,py);
        }     
        if(ADDEKOR)
            addUlar();  
